...
%DIF DEL i2HDM-03-15.tex   Wed Mar 15 14:50:32 2017
%DIF ADD i2HDM-05-09.tex   Mon Jul 10 11:23:15 2017
\pdfoutput=1
\hoffset=-2cm 
\textwidth=17.5cm
\usepackage{amsmath}
...
%DIF < \hline
%DIF < Parameter      & min value &  max value \\ \hline 
%DIF < Mh$_1$ [GeV]   &    10	&     1000  \\ \hline
%DIF < Mh$_2$ [GeV]   &    10	&     1000  \\ \hline
%DIF < Mhc [GeV]      &    10	&    1000  \\ \hline
%DIF < $\lambda_{345}$&  $-8\pi$  &    $8\pi$  \\ \hline
%DIF < $\lambda_2$    &   $-8\pi$ &     $8\pi$   \\ \hline
%DIF < \end{tabular} 
%DIF < \end{center}
%DIF < \caption{Range for the wide scan of the parameter space. \label{tab:wide-scan}}
%DIF < \end{table}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To better delineate the impact of each constraint, we have imposed different cuts on the parameter space sequentially, 
following the classification below:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item[Cut-1:] %%%
\DIFdel{theoretical constraints on the potential }\DIFdelend \DIFaddbegin {\blue \DIFadd{When performing the scan, we took into account the constraints mentioned above
in the following succession. First, we applied only theoretical constraints }\DIFaddend from vacuum stability\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{Eq.(\ref{eq:scalar-pot1}-\ref{eq:scalar-pot2}) and (\ref{eq:l345-vacuum-stab})}%DIFDELCMD < ]%%%
\DIFdel{, 
perturbativityand
unitarity }%DIFDELCMD < [%%%
\DIFdel{Eq.(\ref{eq:unit}-\ref{eq:l345max})}%DIFDELCMD < ]%%%
\DIFdel{;
}%DIFDELCMD < \item[Cut-2:] %%%
\DIFdel{constraints from LEP }%DIFDELCMD < [%%%
\DIFdel{Eq.~(\ref{eq:constr-widths}) and (\ref{eq:dmh12})}%DIFDELCMD < ]%%%
\DIFdel{, EWPT}%DIFDELCMD < [%%%
\DIFdel{Eq.~(\ref{eq:ewpt})}%DIFDELCMD < ] %%%
\DIFdel{and the }\DIFdelend \DIFaddbegin \DIFadd{, 
perturbativity, and unitarity;
second, we applied the collider constraints (LEP, EWPT, }\DIFaddend LHC Higgs data\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{Eq.~(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}%DIFDELCMD < ]%%%
\DIFdel{;
}%DIFDELCMD < \item[Cut-3:] %%%
\DIFdel{constraint on the relic
density  }%DIFDELCMD < [%%%
\DIFdelend \DIFaddbegin \DIFadd{);
last, we placed the upper bound on the DM relic density at }\DIFaddend $\Omega_{\rm DM} h^2 \le 0.1184+2\times 0.0012$ 
\DIFdelbegin %DIFDELCMD < ]%%%
\DIFdel{, where we consider only the upper bound within }\DIFdelend \DIFaddbegin \DIFadd{given by the PLANCK result plus }\DIFaddend 2 standard deviations\DIFdelbegin \DIFdel{;
}%DIFDELCMD < \item[Cut-4:] %%%
\DIFdel{constraints from }\DIFdelend \DIFaddbegin \DIFadd{, 
and took into account the negative results of the }\DIFaddend DM DD searches \DIFdelbegin \DIFdel{from LUX.}%DIFDELCMD < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \vskip %%%
\DIFdel{-1.0cm
}%DIFDELCMD < \hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut12.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut12.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut12.pdf}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdel{-1.1cm
}%DIFDELCMD < \hspace*{1.2cm}%%%
\DIFdel{(a)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.3cm}%%%
\DIFdel{(b)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.2cm}%%%
\DIFdel{(c)
}%DIFDELCMD < \vskip %%%
\DIFdel{0.0cm
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut123456.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut123456.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut123456.pdf}}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdel{-1.1cm
}%DIFDELCMD < \hspace*{1.2cm}%%%
\DIFdel{(d)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.3cm}%%%
\DIFdel{(e)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.2cm}%%%
\DIFdel{(f)
}%DIFDELCMD < \vskip %%%
\DIFdel{0.0cm
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut1234567.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut1234567.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut1234567.pdf}}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdel{-1.05cm
}%DIFDELCMD < \hspace*{1.2cm}%%%
\DIFdel{(g)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.3cm}%%%
\DIFdel{(h)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.2cm}%%%
\DIFdel{(i)
}%DIFDELCMD < \vskip %%%
\DIFdel{0.0cm
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut12345678.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut12345678.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut12345678.pdf}%%%
\DIFdelend \DIFaddbegin \DIFadd{at LUX.}\DIFaddend }
%DIF < 
\DIFdelbegin %DIFDELCMD < \vskip %%%
\DIFdel{-1.05cm
}%DIFDELCMD < \hspace*{1.2cm}%%%
\DIFdel{(j)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.3cm}%%%
\DIFdel{(k)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.2cm}%%%
\DIFdel{(l)
}%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
$(M_{h_1},M_{h^{+}})$ planes. Each row correspond to the Cut-1-4, described in the text: Cut-1 for (a-c) }%DIFDELCMD < [%%%
\DIFdel{Eq.s(\ref{eq:scalar-pot1}-\ref{eq:l345min})}%DIFDELCMD < ]%%%
\DIFdel{; Cut-2 for (d-f) }%DIFDELCMD < [%%%
\DIFdel{Eq.s(\ref{eq:constr-widths}),(\ref{eq:dmh12}),(\ref{eq:ewpt}),(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}%DIFDELCMD < ]%%%
\DIFdel{; Cut-3 for (g-i) }%DIFDELCMD < [%%%
\DIFdel{$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$}%DIFDELCMD < ]%%%
\DIFdel{; Cut-4 for (j-l) }%DIFDELCMD < [%%%
\DIFdel{LUX}%DIFDELCMD < ]%%%
\DIFdel{.
%DIF < 
%DIF < The first row --  the parameter space after the requirement of vacuum stability 
%DIF < [Eq.(\ref{eq:scalar-pot1}-\ref{eq:scalar-pot2},\ref{eq:l345max})], perturbativity  and unitarity
%DIF < [Eq.(\ref{eq:pert}-\ref{eq:l345min})]; the second row --   after additional constraints from LEP
%DIF < [Eq.~(\ref{eq:constr-widths}),(\ref{eq:dmh12})], EWPT [Eq.~(\ref{eq:ewpt}] and the LHC Higgs data
%DIF < [Eq.~(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})]; the third row --   after  additional constraint on relic
%DIF < density [$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$]; the fourth row --  after additional
%DIF < constraints from DM DD searches from LUX.
}%DIFDELCMD < \label{fig:dm-i2hdm}%%%
} 
%DIFAUXCMD
...
\DIFdelFL{-1.0cm
}%DIFDELCMD < \hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut12_z.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut12_z.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut12_z.pdf}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdelFL{-5.2cm
}%DIFDELCMD < \hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdelFL{0.2cm
}%DIFDELCMD < \hspace*{1.4cm}%%%
\DIFdelFL{(a)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.4cm}%%%
\DIFdelFL{(b)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.5cm}%%%
\DIFdelFL{(c)
}%DIFDELCMD < \vskip  %%%
\DIFdelFL{0.0cm
%DIF < 
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut123456_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut123456_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut123456_z.pdf}}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdelFL{-5.2cm
}%DIFDELCMD < \hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%%%
...
\DIFdelFL{(d)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.4cm}%%%
\DIFdelFL{(e)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.5cm}%%%
\DIFdelFL{(f)
}%DIFDELCMD < \vskip %%%
\DIFdelFL{0.0cm
%DIF < 
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut1234567_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut1234567_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut1234567_z.pdf}}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdelFL{-5.2cm
}%DIFDELCMD < \hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%%%
...
\DIFdelFL{(g)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.5cm}%%%
\DIFdelFL{(h)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.6cm}%%%
\DIFdelFL{(i)
}%DIFDELCMD < \vskip %%%
\DIFdelFL{0.0cm
}%DIFDELCMD < {\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut12345678_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut12345678_z.pdf}}%%%
%DIF < 
%DIFDELCMD < {\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut12345678_z.pdf}}%%%
%DIF < 
%DIFDELCMD < \vskip %%%
\DIFdelFL{-5.2cm
}%DIFDELCMD < \hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%%%
...
\DIFdelFL{(j)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.5cm}%%%
\DIFdelFL{(k)}%DIFDELCMD < \hspace*{0.35\textwidth}\hspace*{-1.6cm}%%%
\DIFdelFL{(l)
...
$M_{h^{+}}$. As for Fig.~\ref{fig:dm-i2hdm} each row correspond to the Cut-1-4, described in the text: Cut-1 for (a-c) }%DIFDELCMD < [%%%
\DIFdelFL{Eq}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{s(\ref{eq:scalar-pot1}-\ref{eq:l345min})}%DIFDELCMD < ]%%%
\DIFdelFL{; Cut-2 for (d-f) }%DIFDELCMD < [%%%
\DIFdelFL{Eq}\DIFdelendFL .\DIFdelbeginFL \DIFdelFL{s(\ref{eq:constr-widths}),(\ref{eq:dmh12}),(\ref{eq:ewpt}),(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}%DIFDELCMD < ]%%%
\DIFdelFL{; Cut-3 for (g-i) }%DIFDELCMD < [%%%
\DIFdelFL{$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$}%DIFDELCMD < ]%%%
\DIFdelFL{; Cut-4 for (j-l) }%DIFDELCMD < [%%%
\DIFdelFL{LUX}%DIFDELCMD < ]%%%
\DIFdelendFL . \DIFdelbeginFL %DIFDELCMD < \label{fig:dm-i2hdm-small}%%%
\DIFdelendFL \DIFaddbeginFL \label{fig:scan-simplified}\DIFaddendFL } 
\DIFaddbeginFL \end{center}
\DIFaddendFL \end{figure}

\DIFdelbegin \DIFdel{The results of the scan }\DIFdelend \DIFaddbegin {\blue \DIFadd{The salient features of the results of this scan,
with all three groups of constraints applied successively, 
}\DIFaddend are presented in Fig.~\DIFdelbegin \DIFdel{\ref{fig:dm-i2hdm} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:scan-simplified} on the $(M_{h_1},\lambda_{345})$ projection.
The results are presented }\DIFaddend in the form of \DIFdelbegin \DIFdel{a colour mapof
DM relic density, projected on two-dimensional planes: }\DIFdelend \DIFaddbegin \DIFadd{color map, where the color encodes the relic density.
}{\red \DIFadd{The points of lower relic density are always put on top of large density. }{\bf [\DIFadd{true??}]}} 
\DIFadd{To make the exposition as clear as possible, we decided to keep here only this specific 2D projection. 
The reader can find more information and insights in the Appendix, which contains
a detailed description of three 2D planes, }\DIFaddend $(M_{h_1},\lambda_{345})$\DIFdelbegin \DIFdel{in the first,
}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $(M_{h_1},M_{h_2})$\DIFdelbegin \DIFdel{in the second, and }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend $(M_{h_2},M_{h^{+}})$\DIFdelbegin \DIFdel{in the third column, respectively.
}\DIFdelend \DIFaddbegin \DIFadd{,
upon successive application of several groups of constraints.
}

\DIFaddend The \DIFdelbegin \DIFdel{four rows reproduce the effect of the progressive application of the four Cuts defined above.
In Fig. ~\ref{fig:dm-i2hdm-small} we also present, in the same format, the results of a finer scan, zoomed to the region of low masses, where the range has been restricted to $10$--$200$ GeV for the three masses $M_{h_1},M_{h_2}$ and $M_{h^{+}}$. The latter is the most relevant corner of parameter space for the LHC phenomenology that we will discuss in the next section.
Note that the }\DIFdelend lower bound of $\lambda_{345}$ \DIFdelbegin \DIFdel{presented in these plots }\DIFdelend corresponds to the \DIFdelbegin \DIFdel{lowest limits allowed by unitarity, perturbativity and scalar potential constraints (see Fig.~\ref{fig:par-space1}).
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  where  each row demonstrates the effect of step-by-step application of the
%DIF < experimental and theoretical constraints, while different columns show  2-d planes  for
%DIF < different pairs of the model parameters. The first row -- Figs.\ref{fig:dm-i2hdm} (a-c)
%DIF < presents the parameter space after the requirement of vacuum stability 
%DIF < [Eq.(\ref{eq:scalar-pot1}-\ref{eq:scalar-pot2},\ref{eq:l345max})], perturbativity  and
%DIF < unitarity [Eq.(\ref{eq:pert}-\ref{eq:l345min})] in $(M_{h_1}-\lambda_{345})$,
%DIF < $(M_{h_1}-M_{h_2})$ and  $(M_{h_2}-M_{h^{+}})$ planes  respectively. The second row --  
%DIF < Figs.\ref{fig:dm-i2hdm}(d-f) -- presents results for the same planes  after additional
%DIF < constraints from LEP [Eq.~(\ref{eq:constr-widths}),(\ref{eq:dmh12})], EWPT
%DIF < [Eq.~(\ref{eq:ewpt}] and the LHC Higgs data
%DIF < [Eq.~(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})]. The third row --  
%DIF < Figs.\ref{fig:dm-i2hdm}(g-i) -- presents results after  additional constraint on relic
%DIF < density  [$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$] and, finally, the
%DIF < fourth row --   Figs.\ref{fig:dm-i2hdm}(j-l) -- presents results after additional constraints
%DIF < from DM DD searches from LUX. We will denote the constraints for each row of
%DIF < Fig.\ref{fig:dm-i2hdm} by Cut$\#1$-Cut$\#4$ respectively, noting that each  next cut includes
%DIF < the previous one.
%DIF < At the same time in Figs.\ref{fig:dm-i2hdm-small} we have presented analogous results
%DIF < for the 'zoomed' parameter space  restricted to (10 GeV - 200 GeV) for $M_{h_1},M_{h_2}$ and $M_{h^{+}}$
%DIF < relevant to collider phenomenology as we discuss below.
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{One can see from Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(a)that }\DIFdelend \DIFaddbegin \DIFadd{theoretical lowest limit Eq.~(\ref{eq:scan-limits}).
The upper bound on }\DIFaddend $\lambda_{345}$ \DIFdelbegin \DIFdel{is limited from above,
and the dependence which defines the  shape of this limit as a function of }\DIFdelend \DIFaddbegin \DIFadd{depends on }\DIFaddend $M_{h_1}$ \DIFaddbegin \DIFadd{and }\DIFaddend comes from 
the vacuum stability condition \DIFdelbegin \DIFdel{given by Eq.}\DIFdelend \DIFaddbegin \DIFadd{Eq.~}\DIFaddend (\ref{eq:l345max}). 
\DIFdelbegin \DIFdel{One can also see from
...
the pattern of DM relic density follows the pattern of  $WW$, $ZZ$ }\DIFdelend \DIFaddbegin \DIFadd{Taking into account the collider constraints, }\DIFaddend and \DIFdelbegin \DIFdel{$HH$ thresholds 
presented earlier in Fig.~\ref{fig:1d-mh1-Omega}}\DIFdelend \DIFaddbegin \DIFadd{in particular the invisible Higgs decay
and the $H\gamma\gamma$ coupling strength, restricts $\lambda_{345}$
to $|\lambda_{345}|\leq 0.02$ at $M_{h_1}<M_H/2$}\DIFaddend .

\DIFdelbegin \DIFdel{One can also observe that the effect of Cut-1 plus Cut-2 is quite dramatic: a)
$Br(H\to h_1 h_1)<0.28$ and $\mu^{\gamma\gamma} = 1.14^{+0.38}_{-0.36}$ constraints
require $\lambda_{345}\leq 0.02$ for $M_{h_1}<M_H/2$
}%DIFDELCMD < [%%%
\DIFdel{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(d)}%DIFDELCMD < ]%%%
\DIFdel{; b) LEP constraints require }\DIFdelend \DIFaddbegin \DIFadd{LEP and LHC data also place constraints on the other inert scalars: }\DIFaddend $M_{h_2}\gtrsim 100$ GeV if 
$M_{h_2}-M_{h_1}>8$~GeV\DIFdelbegin %DIFDELCMD < [%%%
\DIFdel{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(e)}%DIFDELCMD < ]%%%
\DIFdel{; c) LEP and LHC Higgs data constraints 
require  $M_{h^+}>70$~GeV, while $M_{h_2}$ is generically excludedbelow $M_Z/2$ }%DIFDELCMD < [%%%
\DIFdel{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(f)}%DIFDELCMD < ]%%%
\DIFdel{.
...
for the whole i2HDM parameter space }%DIFDELCMD < [%%%
\DIFdel{Figs.
~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(g,h)}%DIFDELCMD < ]%%%
\DIFdel{,
but does not have a visible effect in $(M_{h_2},M_{h^{+}})$ plane }%DIFDELCMD < [%%%
\DIFdel{Fig.\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(i)}%DIFDELCMD < ]%%%
\DIFdel{.
Actually the  region with $M_{h_1}<M_Z/2$ is excluded due to the interplay of several constraints.
In the  }\DIFdelend \DIFaddbegin \DIFadd{, $M_{h_2} < M_Z/2$ are generically excluded, and 
...
\DIFadd{The narrow stripe at }\DIFaddend $M_{h_1}<M_H/2$ \DIFdelbegin \DIFdel{region with $|\lambda_{345}|\lesssim 0.02$ as required by LHC Higgs data ,
...
potentially this co-annihilation could provide low enough relic density for $M_{h_1}$ down to about 20 GeV. However, for $M_{h_1}+M_{h_2}< M_Z$
the $Z\to h_1 h_2$ decay is open and contributes significantly to the invisible
Z-boson decay,
that is strongly limited by LEP. As the $Z$-boson partial width for this decay channel
is defined just by }\DIFdelend \DIFaddbegin \DIFadd{surviving after collder data is further cut off once the relic density constraint, in addition,
is taken into account. Indeed, for such a small }\DIFaddend $M_{h_1}$ and \DIFaddbegin \DIFadd{$\lambda_{345}$
and not too small }\DIFaddend $M_{h_2}$ \DIFdelbegin \DIFdel{, since $Z h_1 h_2$ coupling is fixed by the gauge invariance,
the $M_{h_1}+M_{h_2}< M_Z$ parameter space is completely excluded. 
For  $h_1 h_2$ co-annihilation region, this exclusion is equivalent to $M_{h_1}, M_{h_2} \gtrsim M_Z/2$. The $h_1 h_2$ co-annihilation corridor which provides relic density below or equal to PLANCK limit is clearly visible in Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(e,h,k)}\DIFdelend \DIFaddbegin \DIFadd{to prevent $Z \to h_1 h_2$ decays, 
...
This region is again well visible as a protrusion in the $(M_{h_1},M_{h_2})$ plot described in Appendix}\DIFaddend .

The \DIFdelbegin \DIFdel{additional constraint from DM DD from LUX (Cut-4)
removes a substantial portion }\DIFdelend \DIFaddbegin \DIFadd{masses $M_{h_1} > M_H/2$ are not constrained by the relic density,
but the DM DD results from LUX cuts off a part }\DIFaddend of the parameter space\DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{.
This is visible in the last plot in Fig.~\ref{fig:scan-simplified},
for $M_{h_1}\lesssim M_H$ and with }\DIFaddend large and intermediate $|\lambda_{345}|$\DIFdelbegin \DIFdel{values for $M_{h_1}\lesssim M_H$  }%DIFDELCMD < [%%%
\DIFdel{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(j)}%DIFDELCMD < ]%%%
\DIFdelend .
In this \DIFdelbegin \DIFdel{excluded parameter space }\DIFdelend \DIFaddbegin \DIFadd{region, the sizable $|\lambda_{345}|$ simultaneously keeps the relic density below the Planck upper bound
and allows }\DIFaddend the scattering cross section \DIFdelbegin \DIFdel{of $h_1$ on the proton 
is quite large }\DIFdelend \DIFaddbegin \DIFadd{to be enhanced }\DIFaddend due to the Higgs boson exchange\DIFdelbegin \DIFdel{enhanced by $|\lambda_{345}|$, while the relic density is respectively low,
...
So, the LUX cut removes the lowrelic density region, 
...
Once the $h_1 h_1 \to W^+ W^-$ channel is open for positive  $\lambda_{345}$, or $h_1 h_1 \to H H$ channel is open for negative $\lambda_{345}$,
}\DIFdelend \DIFaddbegin \DIFadd{.
The interplay of }{\em \DIFadd{moderately low}} \DIFadd{relic density and a sizable cross section leads to a DD signal
which could have been seen by LUX.
For larger DM masses, the direct annihilation into $WW$, $ZZ$, $HH$ pairs opens up,
and }\DIFaddend the relic density drops \DIFdelbegin \DIFdel{substantially below the PLANCK limit, which makes the rescaling factor 
...
of $M_{h_1}$ for different $\lambda_{345}$ values.}\DIFdelend \DIFaddbegin \DIFadd{further, making LUX insensitive to this region.
...
\DIFaddbegin {\blue \DIFaddend In summary, after all constraints \DIFdelbegin \DIFdel{given by Cut-1-Cut-4,}\DIFdelend \DIFaddbegin \DIFadd{mentioned here and exposed in more detail in Appendix,}}
\DIFaddend we found that the parameter space with  
\begin{equation}
...
space region is already excluded by the upper cut on the relic density\DIFdelbegin \DIFdel{(Cut-3)}\DIFdelend , as one can see
from Fig.~\ref{fig:dm-i2hdm-small}(h): for  $M_{h_2}>100$~GeV, the entire region $M_{h_1}\lesssim
50$~GeV  is excluded by \DIFdelbegin \DIFdel{Cut-3 }\DIFdelend \DIFaddbegin \DIFadd{relic density cut }\DIFaddend combined with previous \DIFdelbegin \DIFdel{cuts (}\DIFdelend \DIFaddbegin \DIFadd{constraints }\DIFaddend including LEPII limits\DIFdelbegin \DIFdel{).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We would also like to point to some features of the scan for  the region of $M_{h_1},M_{h_2}$ and
...
parameters leave only a rather narrow corridor in the  ($M_{h^{+}},M_{h_2}$) plane}\DIFdelend .
...
{\blue \DIFadd{However, it is also }\DIFaddend instructive to explore \DIFdelbegin \DIFdel{also }\DIFdelend the parameter space where both
the upper and lower PLANCK limits are satisfied.
This parameter space region is presented \DIFdelbegin \DIFdel{for the ``full'' }\DIFdelend \DIFaddbegin \DIFadd{in Fig.~\ref{fig:scan-simplified-fitting},
for a wider }\DIFaddend scan 10 GeV $< M_{h_1}, M_{h_2}, M_{h^{+}} < 1000$~GeV \DIFdelbegin \DIFdel{in Fig.~\ref{fig:dm-i2hdm-relic} and}\DIFdelend \DIFaddbegin \DIFadd{and, separately,
}\DIFaddend for the ``zoomed'' \DIFdelbegin \DIFdel{scan }\DIFdelend \DIFaddbegin \DIFadd{region }\DIFaddend 10 GeV $< M_{h_1}, M_{h_2}, M_{h^{+}} < 200$~GeV\DIFdelbegin \DIFdel{in Fig.
~\ref{fig:dm-i2hdm-relic-small}.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_ld345_Omega_zoom-cut123456789_zz-large-monoc.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mh2_Omega_zoom-cut123456789_zz-large-monoc.pdf}
%DIFDELCMD < \vskip %%%
\DIFdel{-1.0cm
}%DIFDELCMD < \hspace*{1cm}%%%
\DIFdel{(a)}%DIFDELCMD < \hspace*{0.55\textwidth}\hspace*{-1.5cm}%%%
\DIFdel{(b)
}%DIFDELCMD < \\
%DIFDELCMD < \\
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mhc_Omega_zoom-cut123456789_zz-large-monoc.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mhc_Mh2_Omega_zoom-cut123456789_zz-large-monoc.pdf}
%DIFDELCMD < \vskip %%%
\DIFdel{-1.0cm
}%DIFDELCMD < \hspace*{1cm}%%%
\DIFdel{(c)}%DIFDELCMD < \hspace*{0.55\textwidth}\hspace*{-1.5cm}%%%
\DIFdel{(d)
}%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{2D projections of the 5D random scan of the i2HDM for
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:dm-i2hdm-relic}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_ld345_Omega_zoom-cut123456789_zz-monoc.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mh2_Omega_zoom-cut123456789_zz-monoc.pdf}
%DIFDELCMD < \vskip %%%
\DIFdel{-0.8cm
}%DIFDELCMD < \hspace*{1cm}%%%
\DIFdel{(a)}%DIFDELCMD < \hspace*{0.55\textwidth}\hspace*{-1.5cm}%%%
...
%DIFDELCMD < \\
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mhc_Omega_zoom-cut123456789_zz-monoc.pdf}%%%
%DIF < 
%DIFDELCMD < \hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mhc_Mh2_Omega_zoom-cut123456789_zz-monoc.pdf}
%DIFDELCMD < \vskip %%%
\DIFdel{-0.8cm
}%DIFDELCMD < \hspace*{1cm}%%%
\DIFdel{(c)}%DIFDELCMD < \hspace*{0.55\textwidth}\hspace*{-1.5cm}%%%
...
{%DIFAUXCMD
\DIFdel{2D projections of the 5D random scan of the i2HDM for the ``zoomed'' parameter space
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:dm-i2hdm-relic-small}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
...
Many interesting features of the i2HDM parameter space arise \DIFdelbegin \DIFdel{from Figs.~\ref{fig:dm-i2hdm-relic} and~\ref{fig:dm-i2hdm-relic-small} }\DIFdelend once the ``correct'' amount of DM relic density is required. 
The $(\lambda_{345},M_{h_1})$ plane \DIFdelbegin \DIFdel{shown in Fig.~\ref{fig:dm-i2hdm-relic}(a) }\DIFdelend displays only two very distinctive $M_{h_1}$ regions\DIFdelbegin \DIFdel{where the relic density satisfies the PLANCK limit within 2 standard deviations. 
The first region at low masses}\DIFdelend \DIFaddbegin \DIFadd{:  
low mass region}\DIFaddend , $53~\mbox{GeV} \lesssim M_{h_1} \lesssim 76~\mbox{GeV}$, \DIFdelbegin \DIFdel{is presented in finer detail in Fig. ~\ref{fig:dm-i2hdm-relic-small}.%DIF < The second one is realised for $M_{h_1}\gtrsim 490\mbox{GeV}$. 
%DIF < The first region presented in details in Fig.~\ref{fig:dm-i2hdm-relic-small} consists of several parts:
It }\DIFdelend \DIFaddbegin \DIFadd{shown also in the zoomed panels,
and the high mass region with $M_{h_1}\gtrsim 490\mbox{GeV}$. Below, we discuss them separately.}}

\subsubsection{\DIFadd{The low mass region}}

\DIFadd{The low mass region }\DIFaddend clearly shows the presence of various \DIFdelbegin \DIFdel{regions }\DIFdelend \DIFaddbegin \DIFadd{regimes }\DIFaddend with specific physical properties:
\begin{itemize}
\item[a)] For $M_{h_1}< M_H/2$, three thin strips are observed. The two symmetric wings for positive and negative $\lambda_{345}$ 
clearly visible in \DIFdelbegin \DIFdel{Fig.~\ref{fig:dm-i2hdm-relic-small}(a) }\DIFdelend \DIFaddbegin \DIFadd{the zoomed $(M_{h_1},\lambda_{345})$ panel }\DIFaddend correspond to DM annihilation via the Higgs boson exchange. 
The thin horizontal line for very small values of $\lambda_{345}$ corresponds to the $h_1h_2$ co-annihilation. 
The latter can also be seen in \DIFdelbegin \DIFdel{Fig.~\ref{fig:dm-i2hdm-relic-small}(b) as a }\DIFdelend \DIFaddbegin \DIFadd{the $(M_{h_1},M_{h_2})$ plots as the }\DIFaddend thin diagonal strip at low $M_{h_2}$
...
\DIFdelbegin \subsection{\DIFdel{The high mass region and the LHC sensitivity}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{The high mass region and the LHC sensitivity}}
\DIFaddend 

\DIFaddbegin {\blue 
\DIFaddend The relic density can also be ``just right'' at large masses $M_{h_1}\gtrsim 490\mbox{ GeV}$, as shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:dm-i2hdm-relic}. 
This high-mass region deserves a separate discussion. 
Its }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:scan-simplified-fitting}. 
The }\DIFaddend most salient feature \DIFaddbegin \DIFadd{of this high-mass region }\DIFaddend is the high degree of degeneracy among the three inert Higgs boson masses\DIFdelbegin \DIFdel{, 
as one can see from }\DIFdelend \DIFaddbegin \DIFadd{.
This is clearly seen in the upper right corner of the $(M_{h_1},M_{h_2})$ plane, as well as in
}\DIFaddend Fig.~\ref{fig:dm-i2hdm-relic} \DIFdelbegin \DIFdel{(b),(c),(d)}\DIFdelend \DIFaddbegin \DIFadd{in the Appendix}\DIFaddend . Numerically we find that the
maximal mass difference among $h_1$, $h_2$ and $h^+$, that we call $\Delta M^{max}$, does not exceed a few GeV. \DIFaddbegin }
\DIFaddend 
...
%DIF < \input{4.1-LHC-DM-rates.tex}
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFdelbegin \subsection{\DIFdel{Dark Matter signatures and rates}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin {\blue 
\DIFaddend The i2HDM exhibits various signatures that are potentially accessible at the LHC.
\DIFdelbegin \DIFdel{First of all, it provides  a }\DIFdelend \DIFaddbegin \DIFadd{They can be generically described as ``mono-object production'', that is,
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\DIFadd{Dark Matter signatures: diagrams and features}}

\subsubsection{\DIFadd{Mono-jet production}}

\DIFadd{The }\DIFaddend mono-jet signature \DIFaddbegin \DIFadd{originates }\DIFaddend from the $pp\to h_1 h_1j$ process,
the Feynman diagrams for which are presented in Fig.~\ref{fig:fd-monojet1}.
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-mono-j1.pdf} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[htb!]
\begin{center}
\includegraphics[width=0.9\textwidth]{fd-mono-j1.pdf} 
\DIFaddendFL \caption{Feynman diagrams for the $pp\to h_1 h_1j$ process
...
\caption{\DIFaddFL{Feynman diagrams for $q\bar{q}\to h_1 h_2g$ ($gq\to h_1 h_2q$) process 
contributing to mono-jet signature.}}
\label{fig:fd-monojet2}
\end{center}
\end{figure}

\DIFaddend There is one more process, namely $q\bar{q}\to
...
jets and/or leptons. 
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-mono-j2.pdf} 
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Feynman diagrams for $q\bar{q}\to h_1 h_2g$ ($gq\to h_1 h_2q$) process 
contributing to mono-jet signature.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:fd-monojet2}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %In spite of the fact that there is one mediator for this process -- the $Z$-boson --
...
\DIFaddbegin \subsubsection{\DIFadd{Mono-$Z$ production}}

%DIF > 
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.9\textwidth]{fd-mono-z.pdf} 
\caption{\DIFaddFL{Feynman diagrams for $q\bar{q}\to h_1 h_1Z$  process 
...
%DIF < 
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-mono-z.pdf} 
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
%
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-mono-h1.pdf} 
%DIFDELCMD < %%%
...
\DIFaddendFL \end{figure}
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-mono-h2.pdf} 
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[h!]
\begin{center}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \includegraphics[width=\textwidth]{fd-vbf.pdf} 
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[htb]
%DIFDELCMD < \hskip %%%
\DIFdel{3cm}%DIFDELCMD < \includegraphics[height=0.4\textheight]{sigma_vs_mh1_new_8tev.pdf} 
%DIFDELCMD < \vskip %%%
\DIFdel{-0.2cm
}%DIFDELCMD < \hskip %%%
\DIFdel{3cm}%DIFDELCMD < \includegraphics[trim={0 0 10.5cm 0},clip,height=0.4\textheight]{sigma_vs_mh1_new_13tev.pdf} 
%DIFDELCMD < \vskip %%%
\DIFdel{-0.2cm
}%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{ Cross sections versus
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:cs}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
%DIF < 
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Mono-object production: rates and comparison}}
\DIFaddend 

\DIFdelbegin \DIFdel{In Fig.~\ref{fig:cs} we present a summary of }\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Implementation and cuts}}

{\blue \DIFadd{When calculating }\DIFaddend the cross sections \DIFdelbegin \DIFdel{versus
...
for the LHCat 8 and 13 TeV.
We have }\DIFdelend \DIFaddbegin \DIFadd{of mono-object production at the LHC,
we }\DIFaddend used the following setup for the process evaluation:\DIFaddbegin }
...
\DIFaddbegin \subsubsection{\DIFadd{Production rates}}

\begin{figure}[htb]
\hskip \DIFaddFL{3cm}\includegraphics[height=0.4\textheight]{sigma_vs_mh1_new_8tev.pdf} 
\vskip \DIFaddFL{-0.2cm
}\hskip \DIFaddFL{3cm}\includegraphics[trim={0 0 10.5cm 0},clip,height=0.4\textheight]{sigma_vs_mh1_new_13tev.pdf} 
\vskip \DIFaddFL{-0.2cm
}\caption{\DIFaddFL{ Cross sections versus
Dark Matter mass, $M_{h_1}$, for  processes
contributing to mono-jet, mono-Z, mono-Higgs and VBF signatures
for the LHC@8 TeV and  LHC@13 TeV.}}
\label{fig:cs}
...
\DIFadd{In Fig.~\ref{fig:cs} we present a summary of the cross sections versus
Dark Matter mass $M_{h_1}$ for all the processes mentioned above, 
which contribute to the mono-jet, mono-$Z$, mono-Higgs and VBF signatures
for the LHC at 8 and 13 TeV.
}

\DIFaddend For the $pp \to  h_1 h_1 j$ processes, the cross section is represented by a solid grey line 
...
\DIFaddbegin \subsection{\DIFadd{Benchmark points}}
...
\DIFaddbegin {\blue \DIFadd{The experience we have gained so far, both in relic density and mono-object cross section calculations,
allows us to discern several qualitatively distinct regimes
of i2HDM and find their representative benchmark points.}}
\DIFaddend In Table~\ref{tab:i2HDMbenchMarks} we present six benchmarks (BM) from the i2HDM  parameter space
together with corresponding observables: DM relic density ($\Omega_{\rm DM} h^2$),
 spin-independent  DM scattering rate on the proton ($\sigma_{SI}^p$)
accompanied with its ratio to the experimental limit from LUX following re-scaling with the relic density: 
$R_{SI}^{LUX} =(\sigma_{SI}^p/\sigma_{SI}^{LUX})\cdot (\Omega_{DM}/\Omega_{DM}^{Planck})$.
...
\DIFdelbegin \DIFdel{In this section, we examine limits }\DIFdelend \DIFaddbegin {\blue \DIFadd{In the previous subsections, we calculated the mono-object production cross sections at the LHC
...
}\DIFaddend from the current 8 TeV and projected 13 TeV LHC data. \DIFaddbegin }
\DIFaddend We concentrate on limits from mono-jet processes, as these are the mono-object signatures
...
\DIFaddbegin \subsubsection{\DIFadd{Implementation and the LHC data used}}

\DIFaddend In order to calculate the limits from the LHC at 8 TeV, we used the {\tt CheckMATE}
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[ht]
%DIFDELCMD < \centering
%DIFDELCMD <  \includegraphics[width=0.65\textwidth]{./lam345_limit_lhc13.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{The limit on $\lambda_{345}$ from  $pp \to h_1 h_1 j$ at 13 TeV
derived from the analysis presented in Fig.~\ref{cc_limit_h1h1} }%DIFDELCMD < \label{lam345_coll_limit}%%%
}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
\caption{\DIFaddFL{The limit on $\lambda_{345}$ from  $pp \to h_1 h_1 j$ at 13 TeV
derived from the analysis presented in Fig.~\ref{cc_limit_h1h1} }\label{lam345_coll_limit}}
\end{figure}


\DIFaddend In Fig.~\ref{lam345_coll_limit} we provide the  limit on $\lambda_{345}$ versus $M_{h_1}$ for different projected luminosities at the LHC@13TeV. This limit is derived from the analysis presented in Fig.~\ref{cc_limit_h1h1} and could be more practical 
for comparison with limits on $\lambda_{345}$ from different experiments.


\DIFdelbegin %DIFDELCMD < \begin{figure}[ht]
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Limits from $pp\rightarrow h_1h_2 j$}}
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF > \section{Constraints on the parameter space due the extended limits}
...
\DIFaddbegin \subsection{\DIFadd{Highlighting the ``always allowed'' regions}}
...
\DIFaddend Besides the AA region it is informative to find and analyse the region
...
\DIFaddbegin \DIFadd{probed}{\blue \DIFadd{---and in the case of negative results, always }\DIFaddend excluded (AE)\DIFdelbegin \DIFdel{or probed }\DIFdelend \DIFaddbegin \DIFadd{---}}\DIFaddend by the above experiments. 
...
\section{\DIFadd{Numerical scan: detailed discussion}}

\DIFadd{To have a complete picture of the properties of i2HDM in the whole parameter space, we have performed a
five-dimensional random scan of the model parameter space with about $10^8$ points, evaluating all relevant
observables and limits mentioned above. The range for 
the model parameters of the scan was chosen according to the Eq.~(\ref{eq:scan-limits}).
}
...
\DIFadd{To better delineate the impact of each constraint, we have imposed different cuts on the parameter space sequentially, 
following the classification below:
}\begin{itemize}
\item[Cut-1:] \DIFadd{theoretical constraints on the potential from vacuum stability 
}[\DIFadd{Eq.(\ref{eq:scalar-pot1}-\ref{eq:scalar-pot2}) and (\ref{eq:l345-vacuum-stab})}]\DIFadd{, perturbativity  and
unitarity }[\DIFadd{Eq.(\ref{eq:unit}-\ref{eq:l345max})}]\DIFadd{;
}\item[Cut-2:] \DIFadd{constraints from LEP }[\DIFadd{Eq.~(\ref{eq:constr-widths}) and (\ref{eq:dmh12})}]\DIFadd{, EWPT
}[\DIFadd{Eq.~(\ref{eq:ewpt})}] \DIFadd{and the LHC Higgs data
}[\DIFadd{Eq.~(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}]\DIFadd{;
}\item[Cut-3:] \DIFadd{constraint on the relic
density  }[\DIFadd{$\Omega_{\rm DM} h^2 \le 0.1184+2\times 0.0012$}]\DIFadd{, where we consider only the upper bound within 2 standard deviations;
}\item[Cut-4:] \DIFadd{constraints from DM DD searches from LUX.
}\end{itemize}


\begin{figure}[htb]
\vskip \DIFaddFL{-1.0cm
}\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut12.pdf}%DIF > 
\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut12.pdf}%DIF > 
\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut12.pdf}%DIF > 
\vskip \DIFaddFL{-1.1cm
}\hspace*{1.2cm}\DIFaddFL{(a)}\hspace*{0.35\textwidth}\hspace*{-1.3cm}\DIFaddFL{(b)}\hspace*{0.35\textwidth}\hspace*{-1.2cm}\DIFaddFL{(c)
}\vskip \DIFaddFL{0.0cm
}{\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut123456.pdf}}%DIF > 
{\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut123456.pdf}}%DIF > 
{\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut123456.pdf}}%DIF > 
\vskip \DIFaddFL{-1.1cm
}\hspace*{1.2cm}\DIFaddFL{(d)}\hspace*{0.35\textwidth}\hspace*{-1.3cm}\DIFaddFL{(e)}\hspace*{0.35\textwidth}\hspace*{-1.2cm}\DIFaddFL{(f)
}\vskip \DIFaddFL{0.0cm
}{\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut1234567.pdf}}%DIF > 
{\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut1234567.pdf}}%DIF > 
{\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut1234567.pdf}}%DIF > 
\vskip \DIFaddFL{-1.05cm
}\hspace*{1.2cm}\DIFaddFL{(g)}\hspace*{0.35\textwidth}\hspace*{-1.3cm}\DIFaddFL{(h)}\hspace*{0.35\textwidth}\hspace*{-1.2cm}\DIFaddFL{(i)
}\vskip \DIFaddFL{0.0cm
}{\hspace*{-0.3cm}\includegraphics[width=0.41\textwidth]{Mh1_ld345_Omega_large-cut12345678.pdf}}%DIF > 
{\hspace*{-1.78cm}\includegraphics[width=0.41\textwidth]{Mh1_Mh2_Omega_large-cut12345678.pdf}}%DIF > 
{\hspace*{-1.77cm}\includegraphics[width=0.41\textwidth]{Mhc_Mh2_Omega_large-cut12345678.pdf}}%DIF > 
\vskip \DIFaddFL{-1.05cm
}\hspace*{1.2cm}\DIFaddFL{(j)}\hspace*{0.35\textwidth}\hspace*{-1.3cm}\DIFaddFL{(k)}\hspace*{0.35\textwidth}\hspace*{-1.2cm}\DIFaddFL{(l)
}\caption{\DIFaddFL{Colour maps of DM relic density for 2D projections of the 5D random scan of the i2HDM:  
each row demonstrates the effect of consequent application
of the experimental and theoretical constraints in the $(M_{h_1},\lambda_{345})$, $(M_{h_1},M_{h_2})$ and 
$(M_{h_1},M_{h^{+}})$ planes. Each row correspond to the Cut-1-4, described in the text: Cut-1 for (a-c) }[\DIFaddFL{Eqs.~(\ref{eq:scalar-pot1}-\ref{eq:l345min})}]\DIFaddFL{; Cut-2 for (d-f) }[\DIFaddFL{Eqs.~(\ref{eq:constr-widths}),(\ref{eq:dmh12}),(\ref{eq:ewpt}),(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}]\DIFaddFL{; Cut-3 for (g-i) }[\DIFaddFL{$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$}]\DIFaddFL{; Cut-4 for (j-l) }[\DIFaddFL{LUX}]\DIFaddFL{.
%DIF > 
%DIF > The first row --  the parameter space after the requirement of vacuum stability 
%DIF > [Eq.(\ref{eq:scalar-pot1}-\ref{eq:scalar-pot2},\ref{eq:l345max})], perturbativity  and unitarity
%DIF > [Eq.(\ref{eq:pert}-\ref{eq:l345min})]; the second row --   after additional constraints from LEP
...
\begin{figure}[htb]
\vskip \DIFaddFL{-1.0cm
}\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut12_z.pdf}%DIF > 
\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut12_z.pdf}%DIF > 
\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut12_z.pdf}%DIF > 
\vskip \DIFaddFL{-5.2cm
}\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\vskip \DIFaddFL{0.2cm
}\hspace*{1.4cm}\DIFaddFL{(a)}\hspace*{0.35\textwidth}\hspace*{-1.4cm}\DIFaddFL{(b)}\hspace*{0.35\textwidth}\hspace*{-1.5cm}\DIFaddFL{(c)
}\vskip  \DIFaddFL{0.0cm
%DIF > 
}{\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut123456_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut123456_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut123456_z.pdf}}%DIF > 
\vskip \DIFaddFL{-5.2cm
}\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\vskip \DIFaddFL{0.2cm
}\hspace*{1.4cm}\DIFaddFL{(d)}\hspace*{0.35\textwidth}\hspace*{-1.4cm}\DIFaddFL{(e)}\hspace*{0.35\textwidth}\hspace*{-1.5cm}\DIFaddFL{(f)
}\vskip \DIFaddFL{0.0cm
%DIF > 
}{\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut1234567_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut1234567_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut1234567_z.pdf}}%DIF > 
\vskip \DIFaddFL{-5.2cm
}\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\vskip \DIFaddFL{0.2cm
}\hspace*{1.4cm}\DIFaddFL{(g)}\hspace*{0.35\textwidth}\hspace*{-1.5cm}\DIFaddFL{(h)}\hspace*{0.35\textwidth}\hspace*{-1.6cm}\DIFaddFL{(i)
}\vskip \DIFaddFL{0.0cm
}{\hspace*{-0.3cm}\includegraphics[width=0.4\textwidth]{Mh1_ld345_Omega_small-cut12345678_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mh1_Mh2_Omega_small-cut12345678_z.pdf}}%DIF > 
{\hspace*{-1.55cm}\includegraphics[width=0.4\textwidth]{Mhc_Mh2_Omega_small-cut12345678_z.pdf}}%DIF > 
\vskip \DIFaddFL{-5.2cm
}\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\hspace*{4.9cm}\includegraphics[width=0.55cm,height=4.cm]{blank.pdf}%DIF > 
\vskip \DIFaddFL{0.2cm
}\hspace*{1.4cm}\DIFaddFL{(j)}\hspace*{0.35\textwidth}\hspace*{-1.5cm}\DIFaddFL{(k)}\hspace*{0.35\textwidth}\hspace*{-1.6cm}\DIFaddFL{(l)
}\caption{\DIFaddFL{Colour maps of DM relic density for 2D projections of the 5D random scan of the
i2HDM for the parameter space restricted to (10 GeV - 200 GeV) for $M_{h_1},M_{h_2}$ and
$M_{h^{+}}$. As for Fig.~\ref{fig:dm-i2hdm} each row correspond to the Cut-1-4, described in the text: Cut-1 for (a-c) }[\DIFaddFL{Eqs.~(\ref{eq:scalar-pot1}-\ref{eq:l345min})}]\DIFaddFL{; Cut-2 for (d-f) }[\DIFaddFL{Eqs.~(\ref{eq:constr-widths}),(\ref{eq:dmh12}),(\ref{eq:ewpt}),(\ref{eq:lhc-higgs-invis}-\ref{eq:lhc-higgs-aa})}]\DIFaddFL{; Cut-3 for (g-i) }[\DIFaddFL{$\Omega_{\rm DM}^{\rm Planck} h^2 \le 0.1184+2\times 0.0012$}]\DIFaddFL{; Cut-4 for (j-l) }[\DIFaddFL{LUX}]\DIFaddFL{.}\label{fig:dm-i2hdm-small}}
\end{figure}

\DIFadd{The results of the  scan are  presented in Fig.~\ref{fig:dm-i2hdm} in the form of a colour map of
...
\DIFadd{One can see from Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(a) that $\lambda_{345}$ is limited from above,
...
\DIFadd{One can also observe that the effect of Cut-1 plus Cut-2 is quite dramatic: a)
$Br(H\to h_1 h_1)<0.28$ and $\mu^{\gamma\gamma} = 1.14^{+0.38}_{-0.36}$ constraints
require $\lambda_{345}\leq 0.02$ for $M_{h_1}<M_H/2$
}[\DIFadd{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(d)}]\DIFadd{; b) LEP constraints require $M_{h_2}\gtrsim 100$ GeV if 
$M_{h_2}-M_{h_1}>8$~GeV }[\DIFadd{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(e)}]\DIFadd{; c) LEP and LHC Higgs data constraints 
require  $M_{h^+}>70$~GeV, while $M_{h_2}$ is generically excluded below $M_Z/2$ }[\DIFadd{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(f)}]\DIFadd{.
The effect from adding the (upper) cut from relic density (Cut-3)
is shown in Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(g-i): one can see that this cut
(combined with the previous ones)
excludes $M_{h_1}<M_Z/2$
for the whole i2HDM parameter space }[\DIFadd{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(g,h)}]\DIFadd{,
but does not have a visible effect in  $(M_{h_2},M_{h^{+}})$ plane }[\DIFadd{Fig.\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(i)}]\DIFadd{.
Actually the  region with $M_{h_1}<M_Z/2$ is excluded due to the interplay of several constraints.
...
\DIFadd{The additional constraint from DM DD from LUX (Cut-4)
removes a substantial portion of the  parameter space for large and intermediate $|\lambda_{345}|$
values for $M_{h_1}\lesssim M_H$  }[\DIFadd{Figs.~\ref{fig:dm-i2hdm}-\ref{fig:dm-i2hdm-small}(j)}]\DIFadd{.
...
\DIFadd{In summary, after all constraints given by Cut-1-Cut-4,
we found that the parameter space with  
...
\DIFadd{is completely excluded. Our results  agree with the results of previous studies on the i2HDM (see, e.g., \mbox{%DIFAUXCMD
\cite{Arhrib:2013ela,Ilnicka:2015jba}
}%DIFAUXCMD
).
In particular, authors of \mbox{%DIFAUXCMD
\cite{Ilnicka:2015jba}
}%DIFAUXCMD
have also stated 
the $M_{h_1},M_{h_2}<45~\mbox{GeV}$ limit.
However we would like to stress that the general exclusion 
for $M_{h_1},M_{h_2}$ }{\it \DIFadd{and for}} \DIFadd{$M_{h^+}$
given by Eq.~(\ref{eq:exclusion1}) is established here for the first time, to the best of our knowledge. In~\mbox{%DIFAUXCMD
\cite{Ilnicka:2015jba}
}%DIFAUXCMD
, for example,  the authors demonstrate 
(see Fig.~6 and Eq.~(18) in \mbox{%DIFAUXCMD
\cite{Ilnicka:2015jba}
}%DIFAUXCMD
) that 
$M_{h^+}$ above $M_H$ is excluded from a  specific scan. Here we find that
...
$M_{h_2}$ are generically allowed to be as light as 45 GeV. One should note that specific regions of the
parameter space can be excluded using di-lepton and missing transverse momentum
signatures: for example, in a recent study~\mbox{%DIFAUXCMD
\cite{Belanger:2015kga}
}%DIFAUXCMD
the authors showed that values of the masses below
$M_{h_1}\lesssim 50$~GeV and  $M_{h_2}\lesssim 140$~GeV can be excluded using this signature,
provided that the mass gap between $M_{h_2}$ and $M_{h_1}$ is large enough. However, we find that this parameter
...
\DIFadd{We would also like to point to some features of the scan for  the region of $M_{h_1},M_{h_2}$ and
$M_{h^{+}}$ above 200 GeV, presented  in Fig.~\ref{fig:dm-i2hdm}. From 
Figs.~\ref{fig:dm-i2hdm}(f),(i),(l), one can see that EWPT constraints require a very  modest
mass split between $M_{h_2}$ and $M_{h^{+}}$ since this mass split is directly related to
values of the $M_{h_2}$ and $M_{h^{+}}$ couplings to  the SM Higgs as well as to the couplings
...
\begin{figure}[htb]
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_ld345_Omega_zoom-cut123456789_zz-large-monoc.pdf}%DIF > 
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mh2_Omega_zoom-cut123456789_zz-large-monoc.pdf}
\vskip \DIFaddFL{-1.0cm
}\hspace*{1cm}\DIFaddFL{(a)}\hspace*{0.55\textwidth}\hspace*{-1.5cm}\DIFaddFL{(b)
}\\
\\
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mhc_Omega_zoom-cut123456789_zz-large-monoc.pdf}%DIF > 
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mhc_Mh2_Omega_zoom-cut123456789_zz-large-monoc.pdf}
\vskip \DIFaddFL{-1.0cm
}\hspace*{1cm}\DIFaddFL{(c)}\hspace*{0.55\textwidth}\hspace*{-1.5cm}\DIFaddFL{(d)
}\caption{\DIFaddFL{2D projections of the 5D random scan of the i2HDM for
 the ``full'' parameter space 10 GeV $< M_{h_1}, M_{h_2}, M_{h^{+}} < 1000$~GeV
satisfying all constraints (Cut-1 to Cut-4) considered above for Fig.~\ref{fig:dm-i2hdm},\ref{fig:dm-i2hdm-small}
plus a lower constraint on relic density given by  Eq.(\ref{eq:planck-limit})}}
...
%DIF > 
\begin{figure}[htb]
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_ld345_Omega_zoom-cut123456789_zz-monoc.pdf}%DIF > 
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mh2_Omega_zoom-cut123456789_zz-monoc.pdf}
\vskip \DIFaddFL{-0.8cm
}\hspace*{1cm}\DIFaddFL{(a)}\hspace*{0.55\textwidth}\hspace*{-1.5cm}\DIFaddFL{(b)
}\\
\\
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mh1_Mhc_Omega_zoom-cut123456789_zz-monoc.pdf}%DIF > 
\hspace*{-0.2cm}\includegraphics[width=0.50\textwidth]{Mhc_Mh2_Omega_zoom-cut123456789_zz-monoc.pdf}
\vskip \DIFaddFL{-0.8cm
}\hspace*{1cm}\DIFaddFL{(c)}\hspace*{0.55\textwidth}\hspace*{-1.5cm}\DIFaddFL{(d)
}\caption{\DIFaddFL{2D projections of the 5D random scan of the i2HDM for the ``zoomed'' parameter space
10 GeV $< M_{h_1}, M_{h_2}, M_{h^{+}} < 200$~GeV satisfying all constraints
(Cut-1 to Cut-4) considered above for Fig.~\ref{fig:dm-i2hdm},\ref{fig:dm-i2hdm-small} plus a
lower constraint on relic density given by  Eq.(\ref{eq:planck-limit})}}
...
{\blue \DIFadd{Finally, for the case, when the relic density is required to fit the PLANCK result within 2 sigma,
