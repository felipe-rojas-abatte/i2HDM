...
Mh1a, ld345a, cs1 = np.genfromtxt ("cs_8TeV_ld345=0.1_old.dat",dtype=float,unpack=True,skip_header=True) 
Mh1b, ld345b, cs2 = np.genfromtxt ("cs_8TeV_ld345=-0.01_old.dat",dtype=float,unpack=True,skip_header=True) 
Mh1c, ld345c, cs3 = np.genfromtxt ("cs_8TeV_ld345_limit_old.dat",dtype=float,unpack=True,skip_header=True) 
#Mh1d, CMcs = np.genfromtxt ("CMcs_8TeV_pp_h1h1j.dat",dtype=float,unpack=True,skip_header=True) 


#CheckMATE analysis files
Mh1y,ld345y,csy,best_SRy,MC_Eventsy,N_eventsy,ry,Accy,Lumy,error_Sy,S_obs,S_expy,cs_limy  = np.genfromtxt ("CM_atlas_1502.01518_old.txt",dtype=float,unpack=True,skip_header=True) 
...
plt.grid()
plt.tight_layout()
plt.savefig('CM_limit_h1h1j_8TeV_old.pdf')
#plt.show()
plt.clf()
